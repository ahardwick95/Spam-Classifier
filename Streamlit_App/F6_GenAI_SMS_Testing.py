...
def Gen_AI_Model_Comparisons():
   ...
   #----------- Right column: adversarial dataset-------------------
    ...
    st.write("""As we can see , on the new dataset that is similar to the original dataset, logistic regression performed well given the stricter imbalance, well naive bayes did not.  
             However, on the adversarial dataset, naive bayes did far better then logistic regression.  Overall, it seems that naive bayes did not generalize well to either new dataset where as logistic regression did generalize well to the new 
